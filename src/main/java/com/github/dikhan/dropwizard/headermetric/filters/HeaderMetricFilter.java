...
public class HeaderMetricFilter implements ContainerRequestFilter {
    ...
    public HeaderMetricFilter(String endPointHit,
                ...
                headerToLookUp -> {
                    ...
                    if (matchedKeyValues != null) {
                        ...
                        headerToLookUpValues.forEach(searchedValue -> {
                            ...
                            // include multiple indexes for the different values; rather the values are all stored in
                            // the first
                            // index concatenated. E,g: x-custom-header-value-1,x-custom-header-value-2. Hence the need
                            // to
                            // look up the first index and search for matches, this is a special case that needs to be
                            // covered.
                                if (matchedKeyValues.get(0).contains(searchedValue)
                                        || matchedKeyValues.contains(searchedValue)) {
                                    String metricName = traceHeadersBundleConfigHelper.getHeaderMetricName(endPointHit,
                                            searchedHeader, searchedValue);
